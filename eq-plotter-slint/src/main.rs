...
fn init_ui_properties(ui: &EqPlotterUi, eq: sync::Arc<sync::RwLock<eq::Eq<f64>>>) {
    let read_eq=eq.read().unwrap();
...
fn init_ui_callbacks(ui: &EqPlotterUi, eq: sync::Arc<sync::RwLock<eq::Eq<f64>>>) {
    ui.global::<Callbacks>().on_request_set_eq_type({
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    init_ui_properties(&ui, eq.clone());
    init_ui_callbacks(&ui, eq.clone());
